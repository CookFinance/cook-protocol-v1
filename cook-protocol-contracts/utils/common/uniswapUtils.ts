...
import { UniswapV2Pair, CKToken, StakingRewards, StandardTokenMock, WETH9, UniswapV2Router02 } from "../contracts";
...
export const calculateEngageQuantities = async(
  ...
  const balanceOnTokenAssetOne = (await assetOne.balanceOf(ckToken.address)).add(extraAssetOne);
  const balanceOnTokenAssetTwo = (await assetTwo.balanceOf(ckToken.address)).add(extraAssetTwo);
...
export const calculateRebalanceQuantity = async(
  ...
  const balanceAssetOne = (await assetOne.balanceOf(ckToken.address)).add(extraAssetOne);
  const balanceAssetTwo = (await assetTwo.balanceOf(ckToken.address)).add(extraAssetTwo);
...
export const calculateTokensInReserve = async(
  ...
  const lpTokenBalance = await stakingRewards.balanceOf(ckToken.address);
