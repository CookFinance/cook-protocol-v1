...
contract CompoundBravoGovernanceAdapter {
    ...
    /**
     * Generates the calldata to register for voting. This is equivalent to delegating to the CKToken address in Compound.
     *
     ...
     * @return bytes                Propose calldata
     */
    function getRegisterCalldata(address _ckToken) external view returns (address, uint256, bytes memory) {
        // delegate(address _delegatee)
        bytes memory callData = abi.encodeWithSignature(DELEGATE_SIGNATURE, _ckToken);
