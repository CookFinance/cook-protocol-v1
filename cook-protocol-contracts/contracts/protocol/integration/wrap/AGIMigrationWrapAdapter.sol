 ...
 * @author Cook Finance
 *
 * "Migration" adapter that burns the AGI tokens currently in the CK in order to remove them
 * from the CK's positions. The AGI token was permanently paused after migration however it still
 * remains as a position in CKs that hold it. By calling the burn function we can zero out a
 * CK's position and remove it from tracking.
 */
