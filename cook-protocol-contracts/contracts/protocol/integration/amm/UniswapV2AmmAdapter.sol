...
contract UniswapV2AmmAdapter is IAmmAdapter {
        ...
        returns (address target, uint256 value, bytes memory data)
    {
        ...
        // Make sure that only up the amount of liquidity tokens owned by the CK Token are redeemed
        uint256 ckTokenLiquidityBalance = pair.balanceOf(ckToken);
        require(_liquidity <= ckTokenLiquidityBalance, "_liquidity must be <= to current balance");
