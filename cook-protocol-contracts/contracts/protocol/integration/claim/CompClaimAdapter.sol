...
contract CompClaimAdapter {
    ...
    /**
     * Generates the calldata for claiming all COMP tokens for the CKToken.
     * https://compound.finance/docs/comptroller#claim-comp
     *
     * @param _ckToken     Set token address
     *
     ...
     */
    function getClaimCallData(ICKToken _ckToken, address /* _rewardPool */) external view returns (address, uint256, bytes memory) {
        bytes memory callData = abi.encodeWithSignature("claimComp(address)", _ckToken);

        return (address(comptroller), 0, callData);
    }

    /**
     * Returns balance of COMP for CKToken
     *
    ...
    function getRewardsAmount(ICKToken _ckToken, address /* _rewardPool */) external view returns(uint256) {
        return comptroller.compAccrued(address(_ckToken));
    }
