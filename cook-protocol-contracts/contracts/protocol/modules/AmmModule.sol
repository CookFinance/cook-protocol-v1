...
contract AmmModule is ModuleBase, ReentrancyGuard {
    ...
    event LiquidityAdded(
        ...
        int256 _ammPoolBalancesDelta,     // Change in CKToken AMM Liquidity Pool token balances
        address[] _components,
        int256[] _componentBalancesDelta  // Change in CKToken component token balances
    );
    ...
    struct ActionInfo {
        ...
        uint256 totalSupply;                        // Total supply of the CKToken
    ...
    /**
     * CK MANAGER ONLY. Adds liquidity to an AMM pool for a specified AMM. User specifies what components and quantity of
        ...
        onlyManagerAndValidCK(_ckToken)
    {
        ...
        emit LiquidityAdded(_ckToken, _ammPool, liquidityTokenDelta, _components, componentsDelta);
    }
    
    /**
     * CK MANAGER ONLY. Adds liquidity to an AMM pool for a specified AMM using a single asset if supported. 
    ...
    /**
     * CK MANAGER ONLY. Removes liquidity from an AMM pool for a specified AMM. User specifies the exact number of 
    ...
    /**
     * CK MANAGER ONLY. Removes liquidity from an AMM pool for a specified AMM, receiving a single component.
        ...
        returns (ActionInfo memory)
    {
        ...
        actionInfo.totalSupply = _ckToken.totalSupply();

        actionInfo.ammAdapter = IAmmAdapter(getAndValidateAdapter(_integrationName));

        actionInfo.liquidityToken = _ammPool;

        actionInfo.preActionLiquidityTokenBalance = IERC20(_ammPool).balanceOf(address(_ckToken));

        actionInfo.preActionComponentBalances = _getTokenBalances(address(_ckToken), _components);

        actionInfo.liquidityQuantity = actionInfo.totalSupply.getDefaultTotalNotional(_poolTokenInPositionUnit);
                                       
        actionInfo.totalNotionalComponents = _getTotalNotionalComponents(_ckToken, _componentUnits);
    ...
    function _validateAddLiquidity(ActionInfo memory _actionInfo) internal view {
        ...
        for (uint256 i = 0; i < _actionInfo.components.length ; i++) {
            ...
            require(
                _actionInfo.ckToken.hasSufficientDefaultUnits(component, _actionInfo.componentUnits[i]),
    ...
    function _validateRemoveLiquidity(ActionInfo memory _actionInfo) internal view {
        ...
        require(
            _actionInfo.ckToken.hasSufficientDefaultUnits(_actionInfo.liquidityToken, _actionInfo.liquidityQuantity),
            "CKToken must own enough liquidity token"
        );
    }
    ...
    function _executeComponentApprovals(ActionInfo memory _actionInfo) internal {
        ...
        for (uint256 i = 0; i < _actionInfo.components.length ; i++) {
            _actionInfo.ckToken.invokeApprove(
    ...
    function _executeAddLiquidity(ActionInfo memory _actionInfo) internal {
        ...
        _actionInfo.ckToken.invoke(targetAmm, callValue, methodData);
    }
    ...
    function _validateMinimumLiquidityReceived(ActionInfo memory _actionInfo) internal view {
        uint256 liquidityTokenBalance = IERC20(_actionInfo.liquidityToken).balanceOf(address(_actionInfo.ckToken));
    ...
    function _validateMinimumUnderlyingReceived(ActionInfo memory _actionInfo) internal view {
        for (uint256 i = 0; i < _actionInfo.components.length; i++) {
            uint256 underlyingBalance = IERC20(_actionInfo.components[i]).balanceOf(address(_actionInfo.ckToken));
    ...
    function _updateComponentPositions(ActionInfo memory _actionInfo) internal returns(int256[] memory) {
        ...
        for (uint256 i = 0; i < _actionInfo.components.length; i++) {
            ...
            (uint256 currentComponentBalance,,) = _actionInfo.ckToken.calculateAndEditDefaultPosition(
                _actionInfo.components[i],
    ...
    function _updateLiquidityTokenPositions(ActionInfo memory _actionInfo) internal returns(int256) {
        ...
        (uint256 currentLiquidityTokenBalance,,) = _actionInfo.ckToken.calculateAndEditDefaultPosition(
            _actionInfo.liquidityToken,
            _actionInfo.totalSupply,
