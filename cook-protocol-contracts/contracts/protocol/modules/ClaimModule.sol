...
contract ClaimModule is ModuleBase {
    ...
    // Array of rewardPool addresses to claim rewards for the CKToken
    mapping(ICKToken => address[]) public rewardPoolList;

    // Array of adapters associated to the rewardPool for the CKToken
     ...
     * @param _ckToken             Address of CKToken
     */
    function updateAnyoneClaim(ICKToken _ckToken) external onlyManagerAndValidCK(_ckToken) {
        anyoneClaim[_ckToken] = !anyoneClaim[_ckToken];
    }
    ...
    function isRewardPool(ICKToken _ckToken, address _rewardPool) public view returns (bool) {
        return rewardPoolList[_ckToken].contains(_rewardPool);
    }
        ...
        returns (bool)
    {
        ...
        return claimSettings[_ckToken][_rewardPool].contains(adapter);
    }
    ...
    function getRewards(
        ...
        returns (uint256)
    {
        IClaimAdapter adapter = _getAndValidateIntegrationAdapter(claimSettings[_ckToken][_rewardPool], _integrationName);
        uint256 rewards = adapter.getRewards(address(_ckToken), _rewardPool);
        return rewards;
    }
    ...
    function _claim(ICKToken _ckToken, address _rewardPool, string calldata _integrationName) internal {
        ...
        emit RewardClaimed(_ckToken, _rewardPool, adapter, rewards);
    }
    ...
    function _removeClaim(ICKToken _ckToken, address _rewardPool, string calldata _integrationName) internal {
        address adapter = getAndValidateAdapter(_integrationName);
        address[] memory _rewardPoolClaimSettings = claimSettings[_ckToken][_rewardPool];

        require(_rewardPoolClaimSettings.contains(adapter), "Integration must be added");
        claimSettings[_ckToken][_rewardPool] = _rewardPoolClaimSettings.remove(adapter);

        if (claimSettings[_ckToken][_rewardPool].length == 0) {
            rewardPoolList[_ckToken] = rewardPoolList[_ckToken].remove(_rewardPool);
            delete claimSettings[_ckToken][_rewardPool];
        }
    }
