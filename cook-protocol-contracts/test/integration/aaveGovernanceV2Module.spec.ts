...
import { AaveGovernanceV2Adapter, CKToken, GovernanceModule } from "@utils/contracts";
...
describe("AaveGovernanceV2Module", () => {
  ...
  describe("when a CKToken has been deployed and issued", async () => {
    let ckToken: CKToken;
    ...
    describe("#delegate", async () => {
      ...
      it("should delegate votes to the correct address", async () => {
        ...
        const initSetVotingPower = await aaveSetup.aaveToken.getPowerCurrent(subjectCKToken.address, 0);

        await subject();

        const finalDelegateeVotingPower = await aaveSetup.aaveToken.getPowerCurrent(subjectDelegatee, 0);
        const finalSetVotingPower = await aaveSetup.aaveToken.getPowerCurrent(subjectCKToken.address, 0);
