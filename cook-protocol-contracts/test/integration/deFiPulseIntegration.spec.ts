...
// import { CKToken } from "@utils/contracts";
...
//     dfpSet = await setup.createCKToken(
...
//     beforeEach(async () => {
//       subjectCKToken = dfpSet.address;
//       subjectIssueQuantity = ether(100);
//       subjectTo = owner.address;
//     });

//     async function subject(): Promise<ContractTransaction> {
//       return setup.issuanceModule.connect(owner.wallet).issue(subjectCKToken, subjectIssueQuantity, subjectTo);
//     }

//     it("should issue the CK", async () => {
//       await subject();
...
//       await setup.issuanceModule.connect(owner.wallet).issue(subjectCKToken, subjectRedeemQuantity, subjectTo);
//     });

//     async function subject(): Promise<ContractTransaction> {
//       return setup.issuanceModule.connect(owner.wallet).redeem(subjectCKToken, subjectRedeemQuantity, subjectTo);
//     }
...
//     it("should have removed the components from the CKToken", async () => {
//       await subject();
//       const ckTokenBalances = await Promise.all(
//         _.map(tokens.tokens, async (token: string) => { return await tokens[token].balanceOf(dfpSet.address); })
//       );
...
//       expect(JSON.stringify(ckTokenBalances)).to.eq(JSON.stringify(expectedBalances));
//     });
...
//       return setup.streamingFeeModule.connect(owner.wallet).accrueFee(subjectCKToken);
//     }

//     it("increases the totalSupply by the correct amount", async () => {
//       const feeState = await setup.streamingFeeModule.feeStates(subjectCKToken);
