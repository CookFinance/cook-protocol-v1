...
describe("CompoundUniswapLeverageDebtIssuance", () => {
  ...
  describe("#issuance", async () => {
    ...
    context("when a default cToken position with 0 supply", async () => {
      ...
      it("should not update the collateral position on the CKToken", async () => {
      ...
      it("should have the correct token balances", async () => {
        ...
        const preSetCEtherBalance = await cEther.balanceOf(subjectCKToken);
        const preMinterUsdcBalance = await setup.usdc.balanceOf(subjectCaller.address);
        const preSetUsdcBalance = await setup.usdc.balanceOf(subjectCKToken);
        ...
        const postSetCEtherBalance = await cEther.balanceOf(subjectCKToken);
        const postMinterUsdcBalance = await setup.usdc.balanceOf(subjectCaller.address);
        const postSetUsdcBalance = await setup.usdc.balanceOf(subjectCKToken);
    ...
    context("when a default cToken position and external borrow position", async () => {
      ...
      cacheBeforeEach(async () => {
        ...
        // Issue 1 CKToken
        issueQuantity = ether(1);
        await debtIssuanceModule.issue(ckToken.address, issueQuantity, owner.address);
      ...
      it("should update the borrow position on the CKToken correctly", async () => {
        const initialPositions = await ckToken.getPositions();
        const previousSecondPositionBalance = await cUsdc.borrowBalanceStored(ckToken.address);
        const ckTotalSupply = await ckToken.totalSupply();

        await subject();

        const expectedSecondPositionUnit = preciseDivCeil(previousSecondPositionBalance, ckTotalSupply).mul(-1);
        const currentPositions = await ckToken.getPositions();
      ...
      it("should have the correct token balances", async () => {
        ...
        const mintQuantity = preciseMul(subjectQuantity, ether(1).add(issueFee));
        const newCEtherPositionUnits = (await ckToken.getPositions())[0].unit;
        const cEtherFlows = preciseMulCeil(mintQuantity, newCEtherPositionUnits);

        await subject();

        const debtPositionUnits = (await ckToken.getPositions())[1].unit;
    ...
    context("when a default cToken position and liquidated borrow position", async () => {
      ...
      cacheBeforeEach(async () => {
        ...
        await cUsdc.liquidateBorrow(ckToken.address, unitsToLiquidate, cEther.address);
      ...
      it("should update the collateral position on the CKToken correctly", async () => {
        ...
        const expectedPostLiquidationUnit = initialPositions[0].unit.sub(preciseDiv(actualSeizedTokens[1], ckTotalSupply));
        expect(initialPositions.length).to.eq(2);
        expect(currentPositions.length).to.eq(2);
        expect(newFirstPosition.component).to.eq(cEther.address);
        expect(newFirstPosition.positionState).to.eq(0); // Default
      ...
      it("should update the borrow position on the CKToken correctly", async () => {
        ...
        await compoundLeverageModule.sync(ckToken.address, true);
        const previousSecondPositionBalance = await cUsdc.borrowBalanceStored(ckToken.address);
        const ckTotalSupply = await ckToken.totalSupply();
    ...
    context("when 2 default positions and 2 external borrow positions", async () => {
      ...
      it("should update the borrow position on the CKToken correctly", async () => {
        ...
        const previousThirdPositionBalance = await cDai.borrowBalanceStored(ckToken.address);
        const expectedThirdPositionUnit = preciseDivCeil(previousThirdPositionBalance, ckTotalSupply).mul(-1);

        const previousFourthPositionBalance = await cUsdc.borrowBalanceStored(ckToken.address);
        const expectedFourthPositionUnit = preciseDivCeil(previousFourthPositionBalance, ckTotalSupply).mul(-1);
      ...
      it("should have the correct token balances", async () => {
        ...
        const preSetWbtcBalance = await setup.wbtc.balanceOf(subjectCKToken);
        const preExternalUsdcBalance = await setup.usdc.balanceOf(cUsdc.address);
        ...
        const daiDebtPositionUnits = (await ckToken.getPositions())[2].unit;
        const usdcDebtPositionUnits = (await ckToken.getPositions())[3].unit;
        ...
        const wbtcFlows = preciseMul(bitcoin(1), ckTotalSupply); // 1 BTC unit
        ...
        const postSetWbtcBalance = await setup.wbtc.balanceOf(subjectCKToken);
        const postExternalUsdcBalance = await setup.usdc.balanceOf(cUsdc.address);
  ...
  describe("#redemption", async () => {
    ...
    context("when a default cToken position and external borrow position", async () => {
      ...
      beforeEach(() => {
        subjectCKToken = ckToken.address;
        subjectQuantity = issueQuantity; // Redeem 1 CKToken so only fee supply is left
        subjectTo = owner.address;
        subjectCaller = owner;
      });
      ...
      it("should update the borrow position on the CKToken correctly", async () => {
        ...
        const ckTotalSupply = await ckToken.totalSupply();
        const previousSecondPositionBalance = (await cUsdc.borrowBalanceStored(ckToken.address)).mul(-1);
        const newSecondPositionNotional = preciseMul(newSecondPosition.unit, ckTotalSupply);
    ...
    context("when a default cToken position and external borrow position with redeem to supply to 0", async () => {
      ...
      it("should update the borrow position on the CKToken correctly", async () => {
        ...
        const ckTotalSupply = await ckToken.totalSupply();
        const currentSecondPositionBalance = await cUsdc.borrowBalanceStored(ckToken.address);
        const usdcSetBalance = await setup.usdc.balanceOf(ckToken.address);
