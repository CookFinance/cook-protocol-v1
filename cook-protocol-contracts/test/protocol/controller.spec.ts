...
describe("Controller", () => {
  ...
  let mockCKTokenFactory: Account;
  let mockPriceOracle: Account;
  let mockCKToken: Account;
  ...
  describe("initialize", async () => {
    ...
    beforeEach(async () => {
      ...
      subjectFactory = [mockCKTokenFactory.address];
    ...
    it("should have a valid factory", async () => {
      ...
      const validFactory = await controller.isFactory(mockCKTokenFactory.address);
  ...
  describe("addCK", async () => {
    let subjectCK: Address;

    beforeEach(async () => {
      if (shouldInitialize) {
        await controller.initialize([], [], [], []);
        await controller.addFactory(mockCKTokenFactory.address);
      }

      subjectCK = mockCKToken.address;
      subjectCaller = mockCKTokenFactory;
    });

    async function subject(): Promise<any> {
      controller = controller.connect(subjectCaller.wallet);
      return controller.addCK(subjectCK);
    }

    it("should be stored in the CK array", async () => {
      await subject();

      const cks = await controller.getCKs();
      expect(cks.length).to.eq(1);
    });

    it("should be returned as a valid CK", async () => {
      await subject();

      const validCK = await controller.isCK(mockCKToken.address);
      expect(validCK).to.eq(true);
    });

    it("should be returned as a valid system contract", async () => {
      await subject();

      const isSystemContract = await controller.isSystemContract(mockCKToken.address);
      expect(isSystemContract).to.eq(true);
    });

    it("should emit the CKAdded event", async () => {
      await expect(subject()).to.emit(controller, "CKAdded").withArgs(subjectCK, subjectCaller.address);
    });

    describe("when the CK already exists", async () => {
      beforeEach(async () => {
        await subject();
      });

      it("should revert", async () => {
        await expect(subject()).to.be.revertedWith("CK already exists");
      });
    });
  ...
  describe("removeCK", async () => {
    ...
    beforeEach(async () => {
      if (shouldInitialize) {
        ...
        await controller.addFactory(mockCKTokenFactory.address);
        // Add CK from factory
        await controller.connect(mockCKTokenFactory.wallet).addCK(mockCKToken.address);
      }

      subjectCKToken = mockCKToken.address;
      subjectCaller = owner;
    });

    async function subject(): Promise<any> {
      return controller.connect(subjectCaller.wallet).removeCK(subjectCKToken);
    }

    it("should remove factory from cks array", async () => {
      await subject();

      const cks = await controller.getCKs();
      expect(cks.length).to.eq(0);
    });

    it("should return false as a valid CK", async () => {
      await subject();

      const isCK = await controller.isCK(mockCKToken.address);
      expect(isCK).to.eq(false);
    });
    ...
    it("should emit the CKRemoved event", async () => {
      await expect(subject()).to.emit(controller, "CKRemoved").withArgs(subjectCKToken);
    });

    describe("when the CK does not exist", async () => {
      beforeEach(async () => {
        await subject();
      });

      it("should revert", async () => {
        await expect(subject()).to.be.revertedWith("CK does not exist");
      });
    });
  ...
  describe("addFactory", async () => {
    ...
    it("should be returned as a valid factory", async () => {
      ...
      const isFactory = await controller.isFactory(mockCKTokenFactory.address);
    ...
    it("should be returned as a valid system contract", async () => {
      ...
      const isSystemContract = await controller.isSystemContract(mockCKTokenFactory.address);
      expect(isSystemContract).to.eq(true);
    });
