...
import { Controller, CKToken, StandardTokenMock, ModuleBaseMock } from "@utils/contracts";
...
describe("CKToken", () => {
  ...
  describe("constructor", async () => {
    ...
    beforeEach(async () => {
      ...
      subjectName = "TestCKToken";
      subjectSymbol = "CKT";
    });

    async function subject(): Promise<any> {
      return await deployer.core.deployCKToken(
    ...
    it("should have the correct name, symbol, controller, multiplier, and manager", async () => {
      const ckToken = await subject();

      const name = await ckToken.name();
      const symbol = await ckToken.symbol();
      const controllerAddress = await ckToken.controller();
      const managerAddress = await ckToken.manager();
      const positionMultiplier = await ckToken.positionMultiplier();
    ...
    it("should have the correct components and componentPositions", async () => {
      ...
      const firstComponent = await ckToken.components(0);
      const secondComponent = await ckToken.components(1);
      const firstComponentVirtualUnit = await ckToken.getDefaultPositionRealUnit(firstComponent);
      const secondComponentVirtualUnit = await ckToken.getDefaultPositionRealUnit(secondComponent);
      const firstComponentExternalModules = await ckToken.getExternalPositionModules(firstComponent);
      const secondComponentExternalModules = await ckToken.getExternalPositionModules(secondComponent);
    ...
    it("should have the correct modules in pending state", async () => {
      ...
      const mockBasicIssuanceModuleState = await ckToken.moduleStates(mockBasicIssuanceModule.address);
      const mockLockedModuleState = await ckToken.moduleStates(mockLockedModule.address);
      expect(mockBasicIssuanceModuleState).to.eq(MODULE_STATE["PENDING"]);
      expect(mockLockedModuleState).to.eq(MODULE_STATE["PENDING"]);
    });
  });

  context("when there is a deployed CKToken", async () => {
    let ckToken: CKToken;
    ...
    beforeEach(async () => {
      ...
      ckToken = await deployer.core.deployCKToken(
        components,
        units,
        modules,
        controller.address,
      ...
      ckToken = ckToken.connect(mockBasicIssuanceModule.wallet);
      await ckToken.initializeModule();

      ckToken = ckToken.connect(mockLockedModule.wallet);
      await ckToken.initializeModule();
    });
    ...
    describe("#invoke", async () => {
      ...
      async function subject(): Promise<any> {
        ckToken = ckToken.connect(subjectCaller.wallet);
        return ckToken.invoke(
      ...
      it("should set the CKTokens approval balance to the spender", async () => {
        await subject();

        const allowance = await firstComponent.allowance(ckToken.address, testSpender);
      ...
      it("should emit the Invoked event", async () => {
        ...
        await expect(subject()).to.emit(ckToken, "Invoked").withArgs(
          subjectTargetAddress,
          subjectValue,
          subjectCallData,
          expectedReturnValue
      ...
      describe("when the module is locked", async () => {
        beforeEach(async () => {
          await ckToken.connect(mockLockedModule.wallet).lock();
      ...
      describe("when sending ETH to a CKToken contract", async () => {
        let transferBalance: BigNumber;
        let ckTokenReceivingETH: CKToken;

        beforeEach(async () => {
          ckTokenReceivingETH = await deployer.core.deployCKToken(
            components,
            units,
            modules,
            controller.address,
          ...
          await manager.wallet.sendTransaction({ to: ckToken.address, value: transferBalance });

          subjectCallData = EMPTY_BYTES;
          subjectTargetAddress = ckTokenReceivingETH.address;
    ...
    describe("#addComponent", async () => {
      ...
      async function subject(): Promise<any> {
        return ckToken.connect(subjectCaller.wallet).addComponent(subjectComponent);
      }

      it("should add to the component array", async () => {
        const prevComponents = await ckToken.getComponents();

        await subject();

        const components = await ckToken.getComponents();

        const expectedComponent = await ckToken.components(components.length - 1);
      ...
      it("should emit the ComponentAdded event", async () => {
        await expect(subject()).to.emit(ckToken, "ComponentAdded").withArgs(subjectComponent);
      });

      describe("when there is a an already existing component", async () => {
        beforeEach(async () => {
          await ckToken.addComponent(subjectComponent);
        });
      ...
      shouldRevertIfCKTokenIsLocked(subject);
    });
    ...
    describe("#removeComponent", async () => {
      ...
      async function subject(): Promise<any> {
        return ckToken.connect(subjectCaller.wallet).removeComponent(subjectComponent);
      }
      ...
      it("should emit the ComponentRemoved event", async () => {
        await expect(subject()).to.emit(ckToken, "ComponentRemoved").withArgs(subjectComponent);
      });
    ...
    describe("#editDefaultPositionUnit", async () => {
      ...
      beforeEach(async () => {
        ...
        await ckToken.connect(subjectCaller.wallet).editPositionMultiplier(multiplier);
      });

      async function subject(): Promise<any> {
        return ckToken.connect(subjectCaller.wallet).editDefaultPositionUnit(subjectComponent, subjectNewUnit);
      }

      it("should properly edit the default position unit", async () => {
        await subject();

        const retrievedUnit = await ckToken.getDefaultPositionRealUnit(subjectComponent);
        expect(retrievedUnit).to.eq(subjectNewUnit);
      });

      it("should emit the DefaultPositionUnitEdited event", async () => {
        await expect(subject()).to.emit(ckToken, "DefaultPositionUnitEdited").withArgs(
          subjectComponent,
          subjectNewUnit
        );
      });
      ...
      describe("when the conversion results in a virtual unit of 0", async () => {
        beforeEach(async () => {
          ...
          await ckToken.connect(subjectCaller.wallet).editPositionMultiplier(hugePositionMultiplier);
        });
    ...
    describe("#addExternalPositionModule", async () => {
      ...
      async function subject(): Promise<any> {
        return ckToken.connect(subjectCaller.wallet).addExternalPositionModule(subjectComponent, subjectExternalModule);
      }

      it("should properly add the module", async () => {
        const prevModules = await ckToken.getExternalPositionModules(subjectComponent);

        await subject();

        const retrievedExternalModules = await ckToken.getExternalPositionModules(subjectComponent);
      ...
      it("should emit the PositionModuleAdded event", async () => {
        await expect(subject()).to.emit(ckToken, "PositionModuleAdded").withArgs(subjectComponent, subjectExternalModule);
      });

      describe("when there is a an already existing component", async () => {
        beforeEach(async () => {
          await ckToken.addExternalPositionModule(subjectComponent, subjectExternalModule);
        });
    ...
    describe("#removeExternalPositionModule", async () => {
      ...
      beforeEach(async () => {
        ...
        await ckToken.connect(subjectCaller.wallet).addExternalPositionModule(subjectComponent, subjectExternalModule);
      });

      async function subject(): Promise<any> {
        return ckToken.connect(subjectCaller.wallet).removeExternalPositionModule(subjectComponent, subjectExternalModule);
      }
      ...
      it("should zero out the data in externalPositions", async () => {
        ...
        const retrievedRealUnit = await ckToken.getExternalPositionRealUnit(subjectComponent, subjectExternalModule);
        const retrievedData = await ckToken.getExternalPositionData(subjectComponent, subjectExternalModule);
      ...
      it("should emit the PositionModuleRemoved event", async () => {
        await expect(subject()).to.emit(ckToken, "PositionModuleRemoved").withArgs(subjectComponent, subjectExternalModule);
      });
    ...
    describe("#editExternalPositionUnit", async () => {
      ...
      async function subject(): Promise<any> {
        return ckToken.connect(subjectCaller.wallet).editExternalPositionUnit(
          subjectComponent,
          subjectModule,
          subjectNewUnit
        );
      ...
      it("should properly edit the external position unit", async () => {
        ...
        const retrievedUnit = await ckToken.getExternalPositionRealUnit(subjectComponent, subjectModule);
        expect(retrievedUnit).to.eq(subjectNewUnit);
      });

      it("should emit the ExternalPositionUnitEdited event", async () => {
        await expect(subject()).to.emit(ckToken, "ExternalPositionUnitEdited").withArgs(
          subjectComponent,
          subjectModule,
          subjectNewUnit
        );
    ...
    describe("#editExternalPositionData", async () => {
      ...
      async function subject(): Promise<any> {
        return ckToken.connect(subjectCaller.wallet).editExternalPositionData(
          subjectComponent,
          subjectModule,
          subjectData
        );
      ...
      it("should emit the ExternalPositionDataEdited event", async () => {
        await expect(subject()).to.emit(ckToken, "ExternalPositionDataEdited").withArgs(
          subjectComponent,
          subjectModule,
          subjectData
        );
    ...
    describe("#editPositionMultiplier", async () => {
      ...
      async function subject(): Promise<any> {
        return ckToken.connect(subjectCaller.wallet).editPositionMultiplier(subjectPositionMultiplier);
      }
      ...
      it("should update the real position units", async () => {
        ...
        const firstPositionRealUnits = await ckToken.getDefaultPositionRealUnit(firstComponent.address);
      ...
      it("should emit the correct PositionMultiplierEdited event", async () => {
        await expect(subject()).to.emit(ckToken, "PositionMultiplierEdited").withArgs(subjectPositionMultiplier);
      });
      ...
      describe("when the positionMultiplier results in a real position unit = 0", async () => {
        ...
        beforeEach(async () => {
          ...
          await ckToken.editDefaultPositionUnit(firstComponent.address, smallPositionUnitValue);
    ...
    describe("#lock", async () => {
      ...
      it("should lock the CKToken", async () => {
        await subject();

        const isLocked = await ckToken.isLocked();
      ...
      it("should set the locker to the module", async () => {
        ...
        const lockerAddress = await ckToken.locker();
      ...
      describe("when the CKToken is already locked", async () => {
        beforeEach(async () => {
          ckToken = ckToken.connect(subjectCaller.wallet);
          await ckToken.lock();
        });
    ...
    describe("#unlock", async () => {
      ...
      async function subject(): Promise<any> {
        ckToken = ckToken.connect(subjectCaller.wallet);
        return ckToken.unlock();
      }

      it("should put the CKToken in an unlocked state", async () => {
        await subject();
      ...
      describe("when the CKToken is already unlocked", async () => {
        beforeEach(async () => {
          await ckToken.unlock();
        });
    ...
    describe("#mint", async () => {
      ...
      async function subject(): Promise<any> {
        ckToken = ckToken.connect(subjectCaller.wallet);
        return ckToken.mint(subjectMintee, subjectQuantity);
      }

      it("should mint the correct quantity to the mintee", async () => {
        await subject();

        const newCKBalance = await ckToken.balanceOf(subjectMintee);
        expect(newCKBalance).to.eq(subjectQuantity);
      });
    ...
    describe("#burn", async () => {
      ...
      beforeEach(async () => {
        ...
        ckToken = ckToken.connect(subjectCaller.wallet);
        await ckToken.mint(subjectMintee, mintQuantity);
      });

      async function subject(): Promise<any> {
        ckToken = ckToken.connect(subjectCaller.wallet);
        return ckToken.burn(subjectMintee, subjectQuantity);
      }
      ...
      it("should reduce the correct quantity of the mintee", async () => {
        ...
        const newCKBalance = await ckToken.balanceOf(subjectMintee);
        expect(newCKBalance).to.eq(mintQuantity.sub(subjectQuantity));
      });
    ...
    describe("#addModule", async () => {
      ...
      async function subject(): Promise<any> {
        ckToken = ckToken.connect(subjectCaller.wallet);
        return ckToken.addModule(subjectModule);
      }

      it("should change the state to pending", async () => {
        await subject();

        const moduleState = await ckToken.moduleStates(subjectModule);
        expect(moduleState).to.eq(MODULE_STATE["PENDING"]);
      });

      it("should emit the ModuleAdded event", async () => {
        await expect(subject()).to.emit(ckToken, "ModuleAdded").withArgs(subjectModule);
      });
    ...
    describe("#removeModule", async () => {
      ...
      beforeEach(async () => {
        ...
        ckToken = ckToken.connect(manager.wallet);
        await ckToken.addModule(moduleMock.address);
        await moduleMock.initializeModuleOnCK(ckToken.address);
      ...
      it("should emit the ModuleRemoved event", async () => {
        await expect(subject()).to.emit(ckToken, "ModuleRemoved").withArgs(subjectModule);
      });
      ...
      describe("when the module is pending", async () => {
        beforeEach(async () => {
          await controller.addModule(pendingModule.address);
          await ckToken.connect(manager.wallet).addModule(pendingModule.address);
    ...
    describe("#removePendingModule", async () => {
      ...
      async function subject(): Promise<any> {
        ckToken = ckToken.connect(subjectCaller.wallet);
        return ckToken.removePendingModule(subjectModule);
      }
      ...
      it("should emit the PendingModuleRemoved event", async () => {
        await expect(subject()).to.emit(ckToken, "PendingModuleRemoved").withArgs(subjectModule);
      });
    ...
    describe("#setManager", async () => {
      ...
      async function subject(): Promise<any> {
        ckToken = ckToken.connect(subjectCaller.wallet);
        return ckToken.setManager(subjectManager);
      }
      ...
      it("should emit the ManagerEdited event", async () => {
        await expect(subject()).to.emit(ckToken, "ManagerEdited").withArgs(subjectManager, manager.address);
      });
    ...
    describe("#initializeModule", async () => {
      ...
      beforeEach(async () => {
        ...
        await ckToken.addModule(subjectModule);
      });

      async function subject(): Promise<any> {
        ckToken = ckToken.connect(subjectCaller.wallet);
        return ckToken.initializeModule();
      }

      it("should add the module to the modules list", async () => {
        await subject();

        const moduleList = await ckToken.getModules();
      ...
      it("should emit the ModuleInitialized event", async () => {
        await expect(subject()).to.emit(ckToken, "ModuleInitialized").withArgs(subjectModule);
      });
    ...
    describe("#getDefaultPositionRealUnit", async () => {
      ...
      beforeEach(async () => {
        ...
        await ckToken.connect(mockBasicIssuanceModule.wallet).editPositionMultiplier(multiplier);
      });

      async function subject(): Promise<BigNumber> {
        return await ckToken.getDefaultPositionRealUnit(subjectComponent);
      }
    ...
    describe("#getExternalPositionRealUnit", async () => {
      ...
      beforeEach(async () => {
        ...
        await ckToken.connect(mockBasicIssuanceModule.wallet).editExternalPositionUnit(
          subjectComponent,
          subjectModule,
          externalUnitToAdd
        );
      ...
      async function subject(): Promise<BigNumber> {
        return await ckToken.getExternalPositionRealUnit(subjectComponent, subjectModule);
      }
    ...
    describe("#getComponents", async () => {
      async function subject(): Promise<Address[]> {
        return await ckToken.getComponents();
      }
    ...
    describe("#getExternalPositionModules", async () => {
      ...
      beforeEach(async () => {
        ...
        await ckToken.connect(mockBasicIssuanceModule.wallet).addExternalPositionModule(
          subjectComponent,
      ...
      async function subject(): Promise<Address[]> {
        return await ckToken.getExternalPositionModules(subjectComponent);
      }
    ...
    describe("#getExternalPositionData", async () => {
      ...
      beforeEach(async () => {
        ...
        await ckToken.connect(subjectCaller.wallet).editExternalPositionData(
          subjectComponent,
          subjectModule,
          expectedData
        );
      ...
      async function subject(): Promise<string> {
        return ckToken.getExternalPositionData(
          subjectComponent,
          subjectModule,
        );
      }
    ...
    describe("#getPositions", async () => {
      ...
      beforeEach(async () => {
        ...
        await subjectCKToken.connect(mockBasicIssuanceModule.wallet).editPositionMultiplier(subjectMultiplier);
      });

      async function subject(): Promise<Position[]> {
        return await subjectCKToken.getPositions();
      }
      ...
      describe("when the CKToken has an external position and the Default virtual unit is 0", async () => {
        ...
        beforeEach(async () => {
          ...
          await subjectCKToken.connect(mockBasicIssuanceModule.wallet).addComponent(externalComponent);
          // Add module to the component
          await subjectCKToken.connect(mockBasicIssuanceModule.wallet).addExternalPositionModule(
            externalComponent,
            externalModule
          );
          await subjectCKToken.connect(mockBasicIssuanceModule.wallet).editExternalPositionUnit(
            externalComponent,
            externalModule,
            externalRealUnit,
          );

          await subjectCKToken.connect(mockBasicIssuanceModule.wallet).editExternalPositionData(
            externalComponent,
            externalModule,
            externalData,
          );
    ...
    describe("#getModules", async () => {
      async function subject(): Promise<Address[]> {
        return await ckToken.getModules();
      }
    ...
    describe("#getTotalComponentRealUnits", async () => {
      ...
      async function subject(): Promise<BigNumber> {
        return await ckToken.getTotalComponentRealUnits(subjectComponent);
      }
    ...
    describe("#isInitializedModule", async () => {
      ...
      async function subject(): Promise<boolean> {
        return await ckToken.isInitializedModule(subjectModule);
      }
    ...
    function shouldRevertIfCKTokenIsLocked(subject: any) {
      describe("when the CKToken is locked", async () => {
        beforeEach(async () => {
          await ckToken.connect(mockLockedModule.wallet).lock();
        });
