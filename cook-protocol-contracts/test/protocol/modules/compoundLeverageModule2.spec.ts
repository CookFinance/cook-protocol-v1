  ...
  describe("#deleverToZeroBorrowBalance", async () => {
    ...
    context("when cETH is collateral asset", async () => {
      ...
      describe("when module is initialized", async () => {
        ...
        it("should wipe out the debt on Compound", async () => {
          ...
          const borrowDebt = (await cDai.borrowBalanceStored(ckToken.address)).mul(-1);
        ...
        it("should remove any external positions on the borrow asset", async () => {
          ...
          const borrowAssetExternalModules = await ckToken.getExternalPositionModules(setup.dai.address);
          const borrowExternalUnit = await ckToken.getExternalPositionRealUnit(
            setup.dai.address,
            compoundLeverageModule.address
          );
          const isPositionModule = await ckToken.isExternalPositionModule(
            setup.dai.address,
            compoundLeverageModule.address
          );
        ...
        it("should update the borrow asset equity on the CKToken correctly", async () => {
          await subject();
        ...
        //   const expectedRepayQuantity = (await cDai.borrowBalanceStored(ckToken.address)).mul(-1);
    ...
    context("when cETH is borrow asset", async () => {
      ...
      cacheBeforeEach(async () => {
        ...
        if (isInitialized) {
          const leverTradeData = tradeMock.interface.encodeFunctionData("trade", [
            ...
            ckToken.address, // Destination address
      ...
      it("should wipe out the debt on Compound", async () => {
        ...
        const borrowDebt = (await cEther.borrowBalanceStored(ckToken.address)).mul(-1);
      ...
      it("should remove any external positions on the borrow asset", async () => {
        ...
        const borrowAssetExternalModules = await ckToken.getExternalPositionModules(setup.weth.address);
        const borrowExternalUnit = await ckToken.getExternalPositionRealUnit(
          setup.weth.address,
          compoundLeverageModule.address
        );
