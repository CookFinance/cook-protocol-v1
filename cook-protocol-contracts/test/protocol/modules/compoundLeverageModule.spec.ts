...
describe("CompoundLeverageModule", () => {
  ...
  describe("#initialize", async () => {
    ...
    const initializeContracts = async () => {
      ...
      await debtIssuanceMock.initialize(ckToken.address);

      if (isAllowlisted) {
        // Add CKToken to allow list
          await compoundLeverageModule.updateAllowedCKToken(ckToken.address, true);
      }
    };
    ...
    describe("when isAllowlisted is true", () => {
      ...
      it("should enable the Module on the CKToken", async () => {
        await subject();
        const isModuleEnabled = await ckToken.isInitializedModule(compoundLeverageModule.address);
        expect(isModuleEnabled).to.eq(true);
      });

      it("should set the Compound settings and mappings", async () => {
        await subject();
        const collateralCTokens = (await compoundLeverageModule.getEnabledAssets(ckToken.address))[0];
        const borrowAssets = (await compoundLeverageModule.getEnabledAssets(ckToken.address))[1];
        const borrowCTokens = await Promise.all(borrowAssets.map(borrowAsset => compoundLeverageModule.underlyingToCToken(borrowAsset)));
        const isCEtherCollateral = await compoundLeverageModule.collateralCTokenEnabled(ckToken.address, cEther.address);
        const isCDaiCollateral = await compoundLeverageModule.collateralCTokenEnabled(ckToken.address, cDai.address);
        const isCDaiBorrow = await compoundLeverageModule.borrowCTokenEnabled(ckToken.address, cDai.address);
        const isCEtherBorrow = await compoundLeverageModule.borrowCTokenEnabled(ckToken.address, cEther.address);
      ...
      it("should enter markets in Compound", async () => {
        await subject();
        const isCEtherEntered = await compoundSetup.comptroller.checkMembership(ckToken.address, cEther.address);
      ...
      it("should register on the debt issuance module", async () => {
        await subject();
        const isRegistered = await debtIssuanceMock.isRegistered(ckToken.address);
      ...
      describe("when debt issuance module is not initialized on CKToken", async () => {
        beforeEach(async () => {
          await ckToken.removeModule(debtIssuanceMock.address);
        });

        afterEach(async () => {
          await ckToken.addModule(debtIssuanceMock.address);
          await debtIssuanceMock.initialize(ckToken.address);
        });
      ...
      describe("when CKToken is not in pending state", async () => {
        beforeEach(async () => {
          ...
          const compoundLeverageModuleNotPendingCKToken = await setup.createCKToken(
            [setup.weth.address],
            [ether(1)],
            [newModule]
          );

          subjectCKToken = compoundLeverageModuleNotPendingCKToken.address;
        });
    ...
    describe("when isAllowlisted is false", async () => {
      ...
      describe("when CKToken is not allowlisted", async () => {
        it("should revert", async () => {
          await expect(subject()).to.be.revertedWith("Not allowed CKToken");
        });
      });

      describe("when any Set can initialize this module", async () => {
        beforeEach(async () => {
          await compoundLeverageModule.updateAnyCKAllowed(true);
        });
  ...
  describe("#lever", async () => {
    ...
    context("when cETH is collateral asset and borrow positions is 0", async () => {
      const initializeContracts = async () => {
        ...
        await oneInchExchangeMockToWeth.addCKTokenAddress(ckToken.address);
        ...
        // Issue 1 CKToken. Note: 1inch mock is hardcoded to trade 590 DAI regardless of Set supply
        const issueQuantity = ether(1);
        destinationTokenQuantity = ether(1);
        await setup.issuanceModule.issue(ckToken.address, issueQuantity, owner.address);
      ...
      describe("when module is initialized", async () => {
        ...
        it("should update the collateral position on the CKToken correctly", async () => {
          const initialPositions = await ckToken.getPositions();

          await subject();

          // cEther position is increased
          const currentPositions = await ckToken.getPositions();
          const newFirstPosition = (await ckToken.getPositions())[0];
        ...
        it("should update the borrow position on the CKToken correctly", async () => {
          const initialPositions = await ckToken.getPositions();
          ...
          // cEther position is increased
          const currentPositions = await ckToken.getPositions();
          const newSecondPosition = (await ckToken.getPositions())[1];

          const expectedSecondPositionUnit = (await cDai.borrowBalanceStored(ckToken.address)).mul(-1);
        ...
        describe("when the leverage position has been liquidated", async () => {
          ...
          cacheBeforeEach(async () => {
            ...
            ethSeized = ether(1);
            await setup.dai.approve(cDai.address, ether(100000));
            await cDai.liquidateBorrow(ckToken.address, preciseMul(ethSeized, liquidationEthPrice), cEther.address);
        ...
        describe("when slippage is greater than allowed", async () => {
          cacheBeforeEach(async () => {
            ...
            await oneInchExchangeMockWithSlippage.addCKTokenAddress(ckToken.address);
    ...
    context("when cETH is borrow asset and borrow positions is 0", async () => {
      ...
      cacheBeforeEach(async () => {
        ...
        await oneInchExchangeMockFromWeth.addCKTokenAddress(ckToken.address);
        ...
        // Issue 1 CKToken. Note: 1inch mock is hardcoded to trade 1 ETH regardless of Set supply
        const issueQuantity = ether(1);
      ...
      it("should update the borrow position on the CKToken correctly", async () => {
        ...
        const expectedSecondPositionUnit = (await cEther.borrowBalanceStored(ckToken.address)).mul(-1);
    ...
    context("when DAI is borrow asset, and is a default position", async () => {
      ...
      it("should update the borrow position on the CKToken correctly", async () => {
        ...
        // cEther position is increased
        const currentPositions = await ckToken.getPositions();
        const newThridPosition = (await ckToken.getPositions())[2];

        const expectedPositionUnit = (await cDai.borrowBalanceStored(ckToken.address)).mul(-1);
  ...
  describe("#sync", async () => {
    ...
    context("when cETH and cDAI are collateral and WETH and DAI are borrow assets", async () => {
      ...
      const initializeContracts = async () => {
        ...
        // Lever both cDAI and cETH in CKToken
        if (isInitialized) {
      ...
      async function subject(): Promise<any> {
        return compoundLeverageModule.connect(subjectCaller.wallet).sync(subjectCKToken, subjectShouldAccrue);
      }
      ...
      describe("when module is initialized", () => {
        ...
        it("should update the collateral positions on the CKToken correctly", async () => {
          const initialPositions = await ckToken.getPositions();
        ...
        it("should update the borrow positions on the CKToken correctly", async () => {
          const initialPositions = await ckToken.getPositions();
          ...
          // cEther position is increased
          const currentPositions = await ckToken.getPositions();
          const newThirdPosition = (await ckToken.getPositions())[2];
          const newFourthPosition = (await ckToken.getPositions())[3];

          const expectedThirdPositionUnit = (await cDai.borrowBalanceStored(ckToken.address)).mul(-1);
          const expectedFourthPositionUnit = (await cEther.borrowBalanceStored(ckToken.address)).mul(-1);
        ...
        describe("when leverage position has been liquidated", async () => {
          ...
          beforeEach(async () => {
            ...
            await setup.dai.approve(cDai.address, ether(100000));
            await cDai.liquidateBorrow(ckToken.address, liquidationRepayQuantity, cEther.address);
          });
  ...
  describe("#removeModule", async () => {
    ...
    it("should remove the Module on the CKToken", async () => {
      await subject();
      const isModuleEnabled = await ckToken.isInitializedModule(compoundLeverageModule.address);
  ...
  describe("#updateAllowedCKToken", async () => {
    let subjectCKToken: Address;
    let subjectStatus: boolean;
    let subjectCaller: Account;

    beforeEach(async () => {
      subjectCKToken = await getRandomAddress();
      subjectStatus = true;
      subjectCaller = owner;
    });

    async function subject(): Promise<any> {
      return compoundLeverageModule.connect(subjectCaller.wallet).updateAllowedCKToken(subjectCKToken, subjectStatus);
    }

    it("should add Set to allow list", async () => {
      await subject();

      const isAllowed = await compoundLeverageModule.allowedCKTokens(subjectCKToken);

      expect(isAllowed).to.be.true;
    });

    it("should emit the correct CKTokenStatusUpdated event", async () => {
      await expect(subject()).to.emit(compoundLeverageModule, "CKTokenStatusUpdated").withArgs(
        subjectCKToken,
        subjectStatus
      );
    });
  ...
  describe("#updateAnyCKAllowed", async () => {
    ...
    async function subject(): Promise<any> {
      return compoundLeverageModule.connect(subjectCaller.wallet).updateAnyCKAllowed(subjectAnySetAllowed);
    }

    it("should remove Set from allow list", async () => {
      await subject();

      const anyCKAllowed = await compoundLeverageModule.anyCKAllowed();

      expect(anyCKAllowed).to.be.true;
    });

    it("should emit the correct AnyCKAllowedUpdated event", async () => {
      await expect(subject()).to.emit(compoundLeverageModule, "AnyCKAllowedUpdated").withArgs(
  ...
  describe("#registerToModule", async () => {
    ...
    const initializeContracts = async function() {
      ...
      await ckToken.addModule(otherIssuanceModule.address);
      await otherIssuanceModule.initialize(ckToken.address);
    };
    ...
    async function subject(): Promise<any> {
      return compoundLeverageModule.registerToModule(subjectCKToken, subjectDebtIssuanceModule);
    }
    ...
    describe("when module is initialized", () => {
      ...
      it("should register on the other issuance module", async () => {
        const previousIsRegistered = await otherIssuanceModule.isRegistered(ckToken.address);
        await subject();
        const currentIsRegistered = await otherIssuanceModule.isRegistered(ckToken.address);
      ...
      describe("when debt issuance module is not initialized on CKToken", async () => {
        beforeEach(async () => {
          await ckToken.removeModule(otherIssuanceModule.address);
        });
  ...
  describe("#addCollateralAssets", async () => {
    ...
    describe("when module is initialized", () => {
      ...
      it("should add the collateral asset to Compound settings and mappings", async () => {
        await subject();
        const collateralCTokens = (await compoundLeverageModule.getEnabledAssets(ckToken.address))[0];
        const isCCompCollateral = await compoundLeverageModule.collateralCTokenEnabled(ckToken.address, cComp.address);
      ...
      it("should enter markets in Compound", async () => {
        await subject();
        const isCEtherEntered = await compoundSetup.comptroller.checkMembership(ckToken.address, cEther.address);
        const isCCompEntered = await compoundSetup.comptroller.checkMembership(ckToken.address, cComp.address);
        expect(isCEtherEntered).to.be.true;
  ...
  describe("#addBorrowAssets", async () => {
    ...
    describe("when module is initialized", () => {
      ...
      it("should add the borrow asset to Compound settings and mappings", async () => {
        ...
        const isCCompBorrow = await compoundLeverageModule.borrowCTokenEnabled(ckToken.address, cComp.address);
  ...
  describe("#moduleIssueHook", async () => {
    ...
    context("when cETH and cDAI are collateral and WETH and DAI are borrow assets", async () => {
      ...
      cacheBeforeEach(async () => {
        ...
        await ckToken.addModule(mockModule.address);
        await ckToken.connect(mockModule.wallet).initializeModule();
      ...
      async function subject(): Promise<any> {
        return compoundLeverageModule.connect(subjectCaller.wallet).moduleIssueHook(subjectCKToken, ZERO);
      }
  ...
  describe("#moduleRedeemHook", async () => {
    ...
    context("when cETH and cDAI are collateral and WETH and DAI are borrow assets", async () => {
      ...
      async function subject(): Promise<any> {
        return compoundLeverageModule.connect(subjectCaller.wallet).moduleRedeemHook(subjectCKToken, ZERO);
      }
  ...
  describe("#componentIssueHook", async () => {
    ...
    let subjectCKToken: Address;
    let subjectCKQuantity: BigNumber;
    ...
    context("when cETH is collateral and DAI is borrow asset", async () => {
      ...
      beforeEach(() => {
        subjectCKToken = ckToken.address;
        subjectCKQuantity = issueQuantity;
      ...
      it("should increase borrowed quantity on the CKToken", async () => {
        ...
        expect(currentDaiBalance).to.eq(preciseMul(borrowQuantity, subjectCKQuantity));
      });
  ...
  describe("#componentRedeemHook", async () => {
    ...
    context("when cETH is collateral and DAI is borrow asset", async () => {
      ...
      cacheBeforeEach(async () => {
        ...
        // Transfer repay quantity to CKToken for repayment
        await setup.dai.transfer(ckToken.address, repayQuantity);
      });
      ...
      it("should decrease borrowed quantity on the CKToken", async () => {
        const previousDaiBalance = await setup.dai.balanceOf(ckToken.address);
