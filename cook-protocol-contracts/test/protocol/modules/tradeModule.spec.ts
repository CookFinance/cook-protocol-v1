...
describe("TradeModule", () => {
  ...
  context("when there is a deployed CKToken with enabled TradeModule", async () => {
    ...
    describe("#initialize", async () => {
      ...
      async function subject(): Promise<any> {
        ...
        return tradeModule.initialize(subjectCKToken);
      }

      it("should enable the Module on the CKToken", async () => {
        await subject();
        const isModuleEnabled = await ckToken.isInitializedModule(tradeModule.address);
    ...
    describe("#trade", async () => {
      ...
      context("when trading a Default component on Kyber", async () => {
        ...
        const initializeContracts = async () => {
          ...
          if (isInitialized) {
            ...
            await tradeModule.initialize(ckToken.address);
          }
          ...
          await setup.issuanceModule.initialize(ckToken.address, mockPreIssuanceHook.address);
        ...
        describe("when the module is initialized", () => {
          ...
          it("should transfer the correct components to the CKToken", async () => {
            const oldDestinationTokenBalance = await destinationToken.balanceOf(ckToken.address);
            ...
            const newDestinationTokenBalance = await destinationToken.balanceOf(ckToken.address);
            expect(newDestinationTokenBalance).to.eq(expectedDestinationTokenBalance);
          });

          it("should transfer the correct components from the CKToken", async () => {
            const oldSourceTokenBalance = await sourceToken.balanceOf(ckToken.address);
            ...
            const newSourceTokenBalance = await sourceToken.balanceOf(ckToken.address);
          ...
          it("should update the positions on the CKToken correctly", async () => {
            const initialPositions = await ckToken.getPositions();
            const initialFirstPosition = (await ckToken.getPositions())[0];
          ...
          describe("when there is a protocol fee charged", async () => {
            ...
            it("should transfer the correct components minus fee to the CKToken", async () => {
              const oldDestinationTokenBalance = await destinationToken.balanceOf(ckToken.address);
            ...
            it("should transfer the correct components from the CKToken to the exchange", async () => {
              const oldSourceTokenBalance = await sourceToken.balanceOf(ckToken.address);
              await subject();
              const totalSourceQuantity = issueQuantity.mul(sourceTokenQuantity).div(ether(1));
              const expectedSourceTokenBalance = oldSourceTokenBalance.sub(totalSourceQuantity);
            ...
            describe("when receive token is more than total position units tracked on CKToken", async () => {
              let extraTokenQuantity: BigNumber;

              beforeEach(async () => {
                extraTokenQuantity = ether(1);
                destinationToken = destinationToken.connect(owner.wallet);
                // Transfer destination token to CKToken
                await destinationToken.transfer(ckToken.address, extraTokenQuantity);
              });
            ...
            describe("when send token is more than total position units tracked on CKToken", async () => {
              let extraTokenQuantity: BigNumber;

              beforeEach(async () => {
                extraTokenQuantity = ether(1);
                sourceToken = sourceToken.connect(owner.wallet);
                // Transfer source token to CKToken
                await sourceToken.transfer(ckToken.address, extraTokenQuantity);
              });
      ...
      context("when trading a Default component on Uniswap", async () => {
        ...
        describe("when path is through multiple trading pairs", async () => {
          ...
          it("should transfer the correct components to the CKToken", async () => {
            const oldDestinationTokenBalance = await setup.dai.balanceOf(ckToken.address);
            const [, , expectedReceiveQuantity] = await uniswapSetup.router.getAmountsOut(
              subjectSourceQuantity,
            ...
            const expectedDestinationTokenBalance = oldDestinationTokenBalance.add(expectedReceiveQuantity);
            const newDestinationTokenBalance = await setup.dai.balanceOf(ckToken.address);
            expect(newDestinationTokenBalance).to.eq(expectedDestinationTokenBalance);
          });
        });
      });
      ...
      context("when trading a Default component with a transfer fee on Uniswap", async () => {
        ...
        it("should transfer the correct components to the CKToken", async () => {
          const oldDestinationTokenBalance = await setup.wbtc.balanceOf(ckToken.address);
          const [, expectedReceiveQuantity] = await uniswapSetup.router.getAmountsOut(
          ...
          const expectedDestinationTokenBalance = oldDestinationTokenBalance.add(expectedReceiveQuantity);
          const newDestinationTokenBalance = await setup.wbtc.balanceOf(ckToken.address);
          expect(newDestinationTokenBalance).to.eq(expectedDestinationTokenBalance);
        });

        it("should transfer the correct components from the CKToken", async () => {
          const oldSourceTokenBalance = await tokenWithFee.balanceOf(ckToken.address);

          await subject();

          const totalSourceQuantity = issueQuantity.mul(sourceTokenQuantity).div(ether(1));
          const expectedSourceTokenBalance = oldSourceTokenBalance.sub(totalSourceQuantity);
          const newSourceTokenBalance = await tokenWithFee.balanceOf(ckToken.address);
          expect(newSourceTokenBalance).to.eq(expectedSourceTokenBalance);
        });
      ...
      context("when trading a Default component on Uniswap version 2 adapter", async () => {
        ...
        describe("when path is through one pair and swaps for exact tokens", async () => {
          ...
          it("should transfer the correct components to the CKToken", async () => {
            const oldSourceTokenBalance = await setup.wbtc.balanceOf(ckToken.address);
            ...
            const newSourceTokenBalance = await setup.wbtc.balanceOf(ckToken.address);
            expect(newSourceTokenBalance).to.eq(expectedSourceTokenBalance);
          });
        });
      ...
      context("when trading a Default component on One Inch", async () => {
        cacheBeforeEach(async () => {
          ...
          await oneInchExchangeMock.addCKTokenAddress(ckToken.address);
          ...
          // Issue 1 CKToken. Note: 1inch mock is hardcoded to trade 1 WBTC unit regardless of Set supply
          issueQuantity = ether(1);
          await setup.issuanceModule.issue(ckToken.address, issueQuantity, owner.address);
        });
        ...
        describe("when function signature does not match 1inch", async () => {
          beforeEach(async () => {
            ...
            subjectData = oneInchExchangeMock.interface.encodeFunctionData("addCKTokenAddress", [ADDRESS_ZERO]);
          });
      ...
      context("when trading a Default component on 0xAPI", async () => {
        cacheBeforeEach(async () => {
          ...
          await zeroExMock.addCKTokenAddress(ckToken.address);
        ...
        describe("when function signature is not supported", async () => {
          beforeEach(async () => {
            // Encode random function
            subjectData = zeroExMock.interface.encodeFunctionData("addCKTokenAddress", [ADDRESS_ZERO]);
          });
