...
import { ContractCallerMock, SingleIndexModule, CKToken } from "@utils/contracts";
...
describe("SingleIndexModule", () => {
  ...
  describe("#trade", async () => {
    ...
    beforeEach(async () => {
      await setup.approveAndIssueCKToken(index, issueAmount);
  ...
  describe("#tradeRemainingWETH", async () => {
    ...
    const initializeContracts = async () => {
      // targetUnits = [ether(60.869565), bitcoin(.02), ether(50)];
      await indexModule.initialize(index.address);
      await setup.approveAndIssueCKToken(index, ether(20));
  ...
  describe("#initialize", async () => {
    ...
    it("should enable the IndexModule on the CKToken", async () => {
      await subject();
    ...
    describe("when the module is already being used", async () => {
      beforeEach(async () => {
        ...
        const newSet = await setup.createCKToken([setup.weth.address], [ether(1)], [indexModule.address]);
    ...
    describe("when CKToken is not in pending state", async () => {
      beforeEach(async () => {
        ...
        const indexModuleNotPendingCKToken = await setup.createCKToken(
          [setup.weth.address],
          [ether(1)],
          [newModule]
        );

        subjectIndex = indexModuleNotPendingCKToken.address;
      });
    ...
    describe("when the CKToken is not enabled on the controller", async () => {
      beforeEach(async () => {
        ...
        subjectIndex = nonEnabledCKToken.address;
      });
