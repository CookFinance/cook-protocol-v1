...
describe("CompoundBravoGovernanceAdapter", () => {
  ...
  describe("#getRegisterCalldata", async () => {
    ...
    async function subject(): Promise<any> {
      return compoundBravoGovernanceAdapter.getRegisterCalldata(subjectCKToken);
    }

    it("should return correct data for registering", async () => {
      const [targetAddress, ethValue, callData] = await subject();
      const expectedCallData = compoundSetup.comp.interface.encodeFunctionData("delegate", [mockCKToken.address]);
