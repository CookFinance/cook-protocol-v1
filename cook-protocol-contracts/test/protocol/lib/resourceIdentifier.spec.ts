...
describe("ResourceIdentifier", () => {
  ...
  describe("#getCKValuer", async () => {
    ...
    async function subject(): Promise<any> {
      return resourceIdentifier.testGetCKValuer(subjectController);
    }

    it("should fetch the correct CK valuer contract", async () => {
      const ckValuer = await subject();
      expect(ckValuer).to.eq(setup.ckValuer.address);
    });
  });
});