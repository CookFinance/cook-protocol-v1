...
const ckController = "0xa4c8d221d8BB851f83aadd0223a8900A6921A349";
...
const issueSetForExactToken = async (ERC20Address: string, account: Signer, amount: Number) => {
  ...
  await exchangeIssuance.approveCKToken(dpiAddress);
...
const issueExactSetFromToken = async (ERC20Address: string, account: Signer, amountCKToken: Number) => {
  // get initial DPI and ERC20 balances
  const dpi = new ethers.Contract(dpiAddress, erc20abi, account);
  const ERC20 = new ethers.Contract(ERC20Address, erc20abi, account);
  const DPIIssueAmount = ethers.utils.parseEther(amountCKToken.toString());
  ...
  await exchangeIssuance.issueExactSetFromToken(
    dpiAddress,
    ERC20Address,
    ethers.utils.parseEther(amountCKToken.toString()),
    amountToken
  );
...
describe("ExchangeIssuance [ @forked-network ]", function () {
  ...
  describe("isCKToken modifier", () => {
    it("Should revert when CKToken is not conform to ICKToken", async () => {
      // deploy ExchangeIssuance.sol
      const exchangeIssuance = await deploy(account);
