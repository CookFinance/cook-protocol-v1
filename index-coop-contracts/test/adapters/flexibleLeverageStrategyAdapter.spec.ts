...
import { CompoundLeverageModule, ContractCallerMock, DebtIssuanceModule, CKToken, ComptrollerMock } from "@utils/contracts/setV2";
...
describe("FlexibleLeverageStrategyAdapter", () => {
  ...
  beforeEach(async () => {
    ckToken = await setV2Setup.createCKToken(
      [cEther.address],
    ...
    await gulpComptrollerMock.addCKTokenAddress(ckToken.address);
    ...
    // Initialize modules
    await debtIssuanceModule.initialize(ckToken.address, ether(1), ZERO, ZERO, owner.address, ADDRESS_ZERO);
    await setV2Setup.issuanceModule.initialize(ckToken.address, ADDRESS_ZERO);
    ...
    await setV2Setup.streamingFeeModule.initialize(ckToken.address, streamingFeeSettings);
    ...
    if ((await ckToken.manager()) == owner.address) {
      await ckToken.connect(owner.wallet).setManager(icManagerV2.address);
    }
  ...
  describe("#constructor", async () => {
    ...
    it("should set the contract addresses", async () => {
      ...
      expect(strategy.ckToken).to.eq(subjectContractSettings.ckToken);
  ...
  describe("#engage", async () => {
    ...
    context("when rebalance notional is greater than max trade size and greater than max borrow", async () => {
      ...
      beforeEach(async () => {
        ...
        await setV2Setup.issuanceModule.issue(ckToken.address, issueQuantity, owner.address);
      ...
      it("should update the borrow position on the CKToken correctly", async () => {
        ...
        const expectedSecondPositionUnit = (await cUSDC.borrowBalanceStored(ckToken.address)).mul(-1);
      ...
      it("should emit Engaged event", async () => {
        ...
        const exchangeRate = await cEther.exchangeRateStored();
        const cEtherBalance = await cEther.balanceOf(ckToken.address);
      ...
      describe("when CKToken has 0 supply", async () => {
        beforeEach(async () => {
          await setV2Setup.issuanceModule.redeem(ckToken.address, ether(1), owner.address);
        });

        it("should revert", async () => {
          await expect(subject()).to.be.revertedWith("CKToken must have > 0 supply");
        });
      });
  ...
  describe("#rebalance", async () => {
    ...
    context("when current leverage ratio is below target (lever)", async () => {
      ...
      describe("when borrow balance is 0", async () => {
        beforeEach(async () => {
          // Repay entire balance of cUSDC on behalf of CKToken
          await cUSDC.repayBorrowBehalf(ckToken.address, MAX_UINT_256);
        });
      ...
      describe("when CKToken has 0 supply", async () => {
        beforeEach(async () => {
          ...
          await debtIssuanceModule.redeem(ckToken.address, ether(1), owner.address);
        });
  ...
  describe("#iterateRebalance", async () => {
    ...
    context("when current leverage ratio is above target and middle of a TWAP rebalance", async () => {
      ...
      describe("when price has moved advantageously towards target leverage ratio", async () => {
        ...
        it("should not update the positions on the CKToken", async () => {
          const initialPositions = await ckToken.getPositions();
          await subject();
          const currentPositions = await ckToken.getPositions();
    ...
    context("when not in a TWAP rebalance", async () => {
      ...
      describe("when greater than max borrow", async () => {
        ...
        it("should update the collateral position on the CKToken correctly", async () => {
          ...
          const previousBorrowBalance = await cUSDC.borrowBalanceStored(ckToken.address);
