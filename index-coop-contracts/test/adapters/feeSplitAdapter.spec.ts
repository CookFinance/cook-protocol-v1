...
import { FeeSplitAdapter, ICManagerV2 } from "@utils/contracts/index";
import { CKToken } from "@utils/contracts/setV2";
...
describe("FeeSplitAdapter", () => {
  ...
  describe("#constructor", async () => {
    ...
    beforeEach(async () => {
      subjectManager = icManagerV2.address;
    ...
    it("should set the correct CKToken address", async () => {
      const feeAdapter = await subject();

      const actualToken = await feeAdapter.ckToken();
      expect(actualToken).to.eq(ckToken.address);
    });
    ...
    it("should set the correct manager address", async () => {
      ...
      expect(actualManager).to.eq(icManagerV2.address);
    });
  ...
  context("when fee adapter is deployed and system fully set up", async () => {
    ...
    beforeEach(async () => {
      ...
      await ckToken.setManager(icManagerV2.address);
    });
    ...
    describe("#accrueFeesAndDistribute", async () => {
      ...
      beforeEach(async () => {
        ...
        await setV2Setup.debtIssuanceModule.issue(ckToken.address, mintedTokens, owner.address);
      ...
      it("should send correct amount of fees to operator and methodologist", async () => {
        const feeState: any = await setV2Setup.streamingFeeModule.feeStates(ckToken.address);
        ...
        const operatorBalance = await ckToken.balanceOf(operator.address);
        const methodologistBalance = await ckToken.balanceOf(methodologist.address);
      ...
      describe("when methodologist fees are 0", async () => {
        ...
        it("should not send fees to methodologist", async () => {
          const preMethodologistBalance = await ckToken.balanceOf(methodologist.address);

          await subject();

          const postMethodologistBalance = await ckToken.balanceOf(methodologist.address);
      ...
      describe("when operator fees are 0", async () => {
        ...
        it("should not send fees to operator", async () => {
          const preOperatorBalance = await ckToken.balanceOf(operator.address);

          await subject();

          const postOperatorBalance = await ckToken.balanceOf(operator.address);
    ...
    describe("#updateStreamingFee", async () => {
      ...
      context("when no timelock period has been set", async () => {
        it("should update the streaming fee", async () => {
          ...
          const feeState = await setV2Setup.streamingFeeModule.feeStates(ckToken.address);

          expect(feeState.streamingFeePercentage).to.eq(subjectNewFee);
        });

        it("should send correct amount of fees to operator and methodologist", async () => {
          const preManagerBalance = await ckToken.balanceOf(icManagerV2.address);
          const feeState: any = await setV2Setup.streamingFeeModule.feeStates(ckToken.address);
          const totalSupply = await ckToken.totalSupply();
          ...
          const postManagerBalance = await ckToken.balanceOf(icManagerV2.address);
    ...
    describe("#updateIssueFee", async () => {
      ...
      context("when no timelock period has been set", async () => {
        it("should update the issue fee", async () => {
          ...
          const issueState: any = await setV2Setup.debtIssuanceModule.issuanceSettings(ckToken.address);
      ...
      context("when 1 day timelock period has been set", async () => {
        ...
        context("when 1 day timelock has elapsed", async () => {
          ...
          it("sets the new streaming fee", async () => {
            await subject();
            const feeStates = await setV2Setup.streamingFeeModule.feeStates(ckToken.address);
    ...
    describe("#updateRedeemFee", async () => {
      ...
      context("when no timelock period has been set", async () => {
        it("should update the redeem fee", async () => {
          ...
          const issuanceState: any = await setV2Setup.debtIssuanceModule.issuanceSettings(ckToken.address);
    ...
    describe("#updateFeeRecipient", async () => {
      ...
      it("sets the new fee recipients", async () => {
        ...
        const streamingFeeState = await setV2Setup.streamingFeeModule.feeStates(ckToken.address);
        const issuanceFeeState = await setV2Setup.debtIssuanceModule.issuanceSettings(ckToken.address);
