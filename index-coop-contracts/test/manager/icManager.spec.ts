...
import { SingleIndexModule, CKToken } from "@utils/contracts/setV2";
...
describe("ICManager", () => {
  ...
  before(async () => {
    ...
    await setV2Setup.issuanceModule.issue(ckToken.address, ckTokensIssued, otherAccount.address);
    ...
    await setV2Setup.streamingFeeModule.updateFeeRecipient(ckToken.address, icManager.address);
    // Transfer ownership to IcManager
    await ckToken.setManager(icManager.address);
  });
  ...
  describe("#constructor", async () => {
    ...
    it("should set the correct CKToken address", async () => {
      ...
      const actualToken = await retrievedICManager.ckToken();
      expect (actualToken).to.eq(subjectCKToken);
    });
  ...
  describe("#accrueFeeAndDistribute", async () => {
    ...
    it("mints the correct amount of new Sets to the operator and methodologist", async () => {
      ...
      const operatorSetBalance = await ckToken.balanceOf(owner.address);
      const methodologistSetBalance = await ckToken.balanceOf(methodologist.address);
  ...
  describe("#updateManager", async () => {
    ...
    describe("when proposed manager hash is already set", async () => {
      ...
      it("should change the manager address", async () => {
        await subject();
        const manager = await ckToken.manager();
  ...
  describe("#addModule", async () => {
    ...
    it("should add the module to the CKToken", async () => {
      await subject();
      const isModule = await ckToken.isPendingModule(subjectModule);
  ...
  describe("#interactModule", async () => {
    ...
    it("should call startRebalance on the index module from the CKToken", async () => {
      await subject();
  ...
  describe("#removeModule", async () => {
    ...
    it("should remove the module from the CKToken", async () => {
      await subject();
      const isModule = await ckToken.isInitializedModule(subjectModule);
