 ...
 * @author Cook Finance
 *
 * Smart contract that enables trustless leverage tokens using the flexible leverage methodology. This adapter is paired with the CompoundLeverageModule from CK
...
contract FlexibleLeverageStrategyAdapter is BaseAdapter {
    ...
    struct ContractSettings {
        ICKToken ckToken;                              // Instance of leverage token
    ...
    ContractSettings public strategy;               // Struct of contracts used in the strategy (CKToken, price oracles, leverage module etc)
    ...
    /**
     * OPERATOR ONLY: Engage to target leverage ratio for the first time. CKToken will borrow debt position from Compound and trade for collateral asset. If target
    ...
    function engage() external onlyOperator {
        ...
        require(engageInfo.ckTotalSupply > 0, "CKToken must have > 0 supply");
    ...
    /**
     * OPERATOR ONLY: Return leverage ratio to 1x and delever to repay loan. This can be used for upgrading or shutting down the strategy. CKToken will redeem
     ...
     * CKToken holders and reduces the interest rate paid for borrowing. Rebalance must not be in progress. Anyone callable
     */
    ...
    /**
     * Get current leverage ratio. Current leverage ratio is defined as the USD value of the collateral divided by the USD value of the CKToken. Prices for collateral
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 collateralRebalanceUnits = chunkRebalanceNotional.preciseDiv(_leverageInfo.action.ckTotalSupply);
    ...
    function _getAndValidateLeveragedInfo(uint256 _slippageTolerance, uint256 _maxTradeSize) internal view returns(LeverageInfo memory) {
        ...
        require(actionInfo.ckTotalSupply > 0, "CKToken must have > 0 supply");
    ...
    function _createActionInfo() internal view returns(ActionInfo memory) {
        ...
        uint256 cTokenBalance = strategy.targetCollateralCToken.balanceOf(address(strategy.ckToken));
        rebalanceInfo.collateralBalance = cTokenBalance.preciseMul(strategy.targetCollateralCToken.exchangeRateStored());
        rebalanceInfo.borrowBalance = strategy.targetBorrowCToken.borrowBalanceStored(address(strategy.ckToken));
        ...
        rebalanceInfo.ckTotalSupply = strategy.ckToken.totalSupply();
    ...
    /**
     * Calculate the current leverage ratio given a valuation of the collateral and borrow asset, which is calculated as collateral USD valuation / CKToken USD valuation
     *
