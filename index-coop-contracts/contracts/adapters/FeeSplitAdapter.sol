...
contract FeeSplitAdapter is BaseAdapter, TimeLockUpgrade {
     ...
     * the balance of the CKToken in the contract after accrual is sufficient for accounting for all collected fees.
     */
    function accrueFeesAndDistribute() public {
        streamingFeeModule.accrueFee(ckToken);
        
        uint256 totalFees = ckToken.balanceOf(address(manager));
        ...
        if (operatorTake > 0) {
            invokeManagerTransfer(address(ckToken), operator, operatorTake);
        }

        if (methodologistTake > 0) {
            invokeManagerTransfer(address(ckToken), methodologist, methodologistTake);
        }
    ...
    function updateStreamingFee(uint256 _newFee) external onlyOperator timeLockUpgrade {
        bytes memory callData = abi.encodeWithSignature("updateStreamingFee(address,uint256)", manager.ckToken(), _newFee);
    ...
    function updateIssueFee(uint256 _newFee) external onlyOperator timeLockUpgrade {
        bytes memory callData = abi.encodeWithSignature("updateIssueFee(address,uint256)", manager.ckToken(), _newFee);
    ...
    function updateRedeemFee(uint256 _newFee) external onlyOperator timeLockUpgrade {
        bytes memory callData = abi.encodeWithSignature("updateRedeemFee(address,uint256)", manager.ckToken(), _newFee);
        invokeManager(address(debtIssuanceModule), callData);
    }
    ...
    function updateFeeRecipient(address _newFeeRecipient) external onlyOperator {
        bytes memory callData = abi.encodeWithSignature("updateFeeRecipient(address,address)", manager.ckToken(), _newFeeRecipient);
        invokeManager(address(streamingFeeModule), callData);
        invokeManager(address(debtIssuanceModule), callData);
    }
