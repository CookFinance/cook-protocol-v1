...
contract ICManager is TimeLockUpgrade, MutualUpgrade {
    ...
    function accrueFeeAndDistribute() public {
        feeModule.accrueFee(ckToken);

        uint256 ckTokenBalance = ckToken.balanceOf(address(this));

        uint256 operatorTake = ckTokenBalance.preciseMul(operatorFeeSplit);
        uint256 methodologistTake = ckTokenBalance.sub(operatorTake);

        ckToken.transfer(operator, operatorTake);

        ckToken.transfer(methodologist, methodologistTake);

        emit FeesAccrued(ckTokenBalance, operatorTake, methodologistTake);
    }

    /**
     * OPERATOR OR METHODOLOGIST ONLY: Update the CKToken manager address. Operator and Methodologist must each call
    ...
    /**
     * OPERATOR ONLY: Interact with a module registered on the CKToken. Cannot be used to call functions in the
    ...
    /**
     * METHODOLOGIST ONLY: Update the streaming fee for the CKToken. Subject to timelock period agreed upon by the
    ...
    function updateStreamingFee(uint256 _newFee) external timeLockUpgrade onlyMethodologist {
        feeModule.updateStreamingFee(ckToken, _newFee);
    }
    ...
    function updateFeeRecipient(address _newFeeRecipient) external mutualUpgrade(operator, methodologist) {
        feeModule.updateFeeRecipient(ckToken, _newFeeRecipient);
    }
