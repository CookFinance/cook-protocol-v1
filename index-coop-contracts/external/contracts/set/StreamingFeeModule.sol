...
contract StreamingFeeModule is ModuleBase, ReentrancyGuard {
     ...
     *
     * @param  _ckToken          Address of Set to have feeState updated
    ...
    function _feeRecipient(ICKToken _set) internal view returns (address) {
        return feeStates[_set].feeRecipient;
    }

    function _lastStreamingFeeTimestamp(ICKToken _set) internal view returns (uint256) {
        return feeStates[_set].lastStreamingFeeTimestamp;
    }

    function _maxStreamingFeePercentage(ICKToken _set) internal view returns (uint256) {
        return feeStates[_set].maxStreamingFeePercentage;
    }

    function _streamingFeePercentage(ICKToken _set) internal view returns (uint256) {
