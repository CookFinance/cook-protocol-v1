...
contract SingleIndexModule is ModuleBase, ReentrancyGuard {
     ...
     */
    function trade(address _component) external nonReentrant onlyAllowedTrader(msg.sender) virtual {
     ...
     * @param _component            Component to trade
     */
    function tradeRemainingWETH(address _component) external nonReentrant onlyAllowedTrader(msg.sender) virtual {
        require(msg.sender == tx.origin, "Caller must be EOA Address");
     ...
     */
    function updateTraderStatus(address[] calldata _traders, bool[] calldata _statuses) external onlyManagerAndValidCK(index) {
